...
export default class MarkdownRenderer {
  ...
  render(): string {
    ...
    let markdownHTMLDocument: Document = new DOMParser().parseFromString(preRenderedMarkdownFile, "text/html")
  ...
  _addCopyElements(content: string): string {
    return content.replaceAll(/<pre>/g, (value: string) => {
	...
	_flatTagsFromConfig(): string[] {
		return this.attributeConfig.map((elementConfig: any) => elementConfig.elementTags.toString()).toString().split(',')
	}

	_getConfigFromTag(tag: string): object {
		return this.attributeConfig.map((e: any)=>{
			if (e.elementTags.includes(tag))
				return e
		}).filter(e=>e)[0]
	}
		...
		HTMLElements.forEach((elementsObject: any) => {
			...
			if (elementsObject.elements) {
				elementsObject.elements.forEach((element: Element) => {
					...
					// Add classes
					if (elementConfig.class) elementConfig.class.forEach((cls: string) => {
